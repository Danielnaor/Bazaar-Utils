 ...
 *
 * <p>Hypixel labels the lore items from the market-taker's perspective: "Create Buy Order"
 * shows the current sell offers (asks) and "Create Sell Offer" shows the current buy orders
 * (bids). The parser reads them accordingly — swapped relative to their item names.
 *
 * <p>One product per container load. {@link #commitProduct} calls
 * {@link com.github.mkram17.bazaarutils.data.bazaar.pipeline.FillInference#applyAll}
 * immediately after applying levels, with no batching. A
 * {@link com.github.mkram17.bazaarutils.events.bazaar.data.BazaarDataUpdateEvent} is posted
 * only when book or fill state actually changed.
 */
@DataSource
