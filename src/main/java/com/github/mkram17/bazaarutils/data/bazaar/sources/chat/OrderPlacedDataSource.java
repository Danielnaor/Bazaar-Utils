 ...
 *
 * <p>Buy price: {@code truncate(totalCoins / amount)}. Sell price: the chat reports a
 * post-tax total; pre-tax per-unit is recovered as
 * {@code truncate(truncate(totalCoins / amount) / ((100 − tax) / 100))}. Both paths
 * then prefer the exact fractional price from {@link TransactionAPI} when a confirmation
 * screen was observed — that price is more precise than the chat-derived value for
 * large orders where Hypixel rounds the displayed total.
 *
 * <p>Before writing storage, {@link com.github.mkram17.bazaarutils.data.bazaar.book.ProductData#estimateCrossVolume}
 * checks how much of the new order would immediately cross existing offers. Crossed volume
 * sets the initial status to {@link OrderStatus.Partial} or {@link OrderStatus.Filled};
 * uncrossed volume sets it to {@link OrderStatus.Set}. The book mutation places the
 * uncrossed remainder on the own side and walks the crossed volume off the opposite side
 * atomically. An invalid recovered sell price aborts the entire operation.
 */
@DataSource
