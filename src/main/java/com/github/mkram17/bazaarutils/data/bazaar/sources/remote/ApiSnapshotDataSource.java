 ...
 *
 * <p>Per product: incoming bid and ask levels are merged via supersession rules; stale
 * API-sourced levels absent from the snapshot are evicted; fill is inferred for orders
 * whose price level vanished from the post-apply book or whose unfilled amount exceeds
 * the surviving level volume.
 *
 * <p>Fill inference eligibility requires that an order is either session-confirmed (placed
 * or screen-reconciled at or after mod init time) or has its price level absent from the
 * post-apply book. Resumed orders at a still-present level are excluded — their stored
 * {@code unfilledAmount} is not a trustworthy delta baseline without session history. Resumed
 * orders at a vanished level pass unconditionally: level absence is authoritative market truth
 * regardless of session history.
 *
 * <p>Inferences across the full product set are accumulated during the loop and committed
 * in a single {@link FillInference#applyAll} call — one storage write per API tick regardless
 * of product count.
 *
 * <p>{@link com.github.mkram17.bazaarutils.data.bazaar.book.ProductData#hasReceivedSnapshot()}
 * is read before {@link #snapshotProduct} and stamped after, ensuring the first snapshot
 * establishes a baseline without triggering premature inference on resumed orders.
 */
@DataSource
