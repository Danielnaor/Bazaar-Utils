 ...
 *
 * <p>Fires once per minute. All placement sources stamp {@code expiresAt = placedAt + 7 days}
 * at creation, so expiry is deterministic from local state — no Orders screen observation is
 * required. {@code OrdersScreenDataSource} serves as a same-tick fallback when the player opens
 * the Orders screen in the narrow window between the real expiry time and the next cron fire,
 * where the {@code §8Expired!} lore token triggers the same transition.
 *
 * <p>Double-expiry is impossible: both paths gate on {@link Order#isActive()}, which excludes
 * orders already carrying {@link OrderStatus.Expired}.
 *
 * <p>Per expired order the sequence is: book decrement at
 * {@link Order#unfilledAmount()} with {@code terminal=true} (Hypixel removed the unfilled
 * volume); then a single atomic storage write advancing {@code lastUpdatedAt} to now (which
 * prevents {@code OrdersScreenDataSource}'s eviction grace window from double-processing these
 * orders); then {@link com.github.mkram17.bazaarutils.events.bazaar.UserOrderEvent.Expired}
 * per order and
 * {@link com.github.mkram17.bazaarutils.events.bazaar.data.BazaarDataUpdateEvent} per affected
 * product.
 */
@Module
