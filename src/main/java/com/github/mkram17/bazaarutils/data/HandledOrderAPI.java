 ...
 *
 * <p>Consumed by {@link com.github.mkram17.bazaarutils.data.bazaar.pipeline.OrderResolver}
 * to bias order matching toward the order the player physically interacted with, rather than
 * relying on price/volume approximation. Cleared on every Orders page open to prevent stale
 * state from persisting across interactions.
 *
 * <p>Button semantics for fully-filled BUY orders: left-click resolves to CLAIM (collect items),
 * right-click resolves to OPTIONS (flip). All other orders with claimable fill go to CLAIM on
 * any click. Non-claimable selectable orders go to OPTIONS; clicks on non-selectable slots clear
 * the selection.
 */
@Module
