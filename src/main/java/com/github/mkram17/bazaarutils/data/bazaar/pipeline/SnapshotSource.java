    ...
    protected final void commitAll(
        ...
        if (anyOrderChange) {
            ...
            if (NotificationType.ORDER_LIFECYCLE.isEnabled()) {
                ...
                PlayerLogger.debug("%s — %s committed: Δ%d placed, Δ%d evicted, Δ%d updated, Δ%d expired, Δ%d price-corrected, Δ%d reanchored".formatted(
                        origin.describe(), productId, placed, evicted, updated, expired, corrected, reanchored), NotificationType.ORDER_LIFECYCLE, log());
            }
        }
