    ...
    protected final void commitAll(
        ...
        if (anyOrderChange) {
            ...
            if (NotificationType.ORDERDATA.isEnabled()) {
                ...
                PlayerActionUtil.notifyAll("%s — %s committed: Δ%d placed, Δ%d evicted, Δ%d updated, Δ%d expired, Δ%d price-corrected, Δ%d reanchored".formatted(
                        origin.describe(), productId, placed, evicted, updated, expired, corrected, reanchored), NotificationType.ORDERDATA);
            }
        }
