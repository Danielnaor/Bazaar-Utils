 ...
 *
 * <p>The message carries only the product name and original volume — no price. Candidate orders
 * are located by matching {@code originalAmount == volume}; when multiple candidates exist,
 * {@link OrderResolver#selectFillTarget} picks the one at competitive book position, with FIFO
 * as a tiebreak.
 *
 * <p>Any unfilled volume still tracked on the order represents fill that occurred between the
 * last screen reconciliation and this message. That remainder is closed against the book as a
 * terminal decrement before the fill transition is committed.
 *
 * <p>When no stored order matches — typically a co-op member's order appearing on the shared
 * Orders page — a synthetic {@link Order} is inserted as {@link OrderSlotPosition.OffScreen}
 * with price {@code 0.0} and {@code coopOrder = true}. {@code OrdersScreenDataSource} will
